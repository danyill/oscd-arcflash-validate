...
:uri-plugin: https://github.com/danyill/oscd-arcflash-validate
:uri-ci-bundle: https://danyill.github.io/oscd-arcflash-validate/oscd-arcflash-validate.js
:uri-material-icons: https://fonts.google.com/icons
:uri-openscd-core-plugin-demo: https://danyill.github.io/oscd-arcflash-validate/index.deploy.html

== What is this?

IMPORTANT: This is a prototype and is not intended for production use.

This plugin is intended to allow a user to achieve only one specific function within an IEC 61850 SCL file:

* Replace a range of IEDs with one specific IED (transferring only the IED name)
* Transfer all ExtRef sections to the new IED 
...
. The user selects a single IED as the "template"

. The user then selects a range of other IEDs to replace.

. Each IED to be replaced is turned into an exact copy of the "template". With the following changes:

* The IED name attribute is transferred
* All Inputs sections are transferred
...
For a local resource, begins without a forward slash, e.g. `plugins/oscd-arcflash-validate/dist/oscd-arcflash-validate.js`. 
...
----
{
  ...
  "active": true, // <.>
  "requireDoc": true, // <.>
  "src": "/dist/oscd-arcflash-validate.js" // <.>
}
----
<.> Name of the plugin which appears in the editor menu at top of screen
...
<.> Whether a document must be loaded for this plugin to be available
<.> URL which can be a local or remote resource. 
For a local resource, begins with a forward slash, e.g. `plugins/oscd-arcflash-validate/dist/oscd-arcflash-validate.js`. 
